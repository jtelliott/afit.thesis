...
This endeavor utilizes two models for forecasting: naive and dynamic regression. The former is a far simpler technique and is only used as a baseline. The latter is a bit more complex. Dynamic regression has two major components, regression and time-series, each with their own assumptions and requirements. Regression models with multiple predictor variables assume independence of the predictors (also called regressors or exogeneous variables). All regression models assume errors normally and independently distributed around zero with constant variance. 
